...
if (!container) {
  ...
  const camera = new THREE.OrthographicCamera(-scale, scale, scale, -scale, 0.1, 50);
  ...
  function rounded(w, h, d, radius, color, extra = {}) {
    const geo = new RoundedBoxGeometry(w, h, d, 4, radius);
    const mat = new THREE.MeshStandardMaterial({ color, roughness: 0.45, metalness: 0.08, ...extra });
  ...
  const antennaTip = new THREE.Mesh(new THREE.SphereGeometry(0.075, 16, 16), antennaTipMat);
